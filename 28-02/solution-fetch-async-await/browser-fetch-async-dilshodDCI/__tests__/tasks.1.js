...
describe('Browser Fetch Async', () => {
    ...
    it("Promise handling the model popup should be converted to `async/await`", async () => {
        const indexFileContents = fs
            .readFileSync("./src/scripts/index.js")
            .toString("utf-8")
            .replace(/ /g, "");
    ...
    it("'animation-duration' property should be defined for 'Continue' button", async () => {
        ...
        const continueBtnAnimation = await page.$eval('#continue', (el) => getComputedStyle(el).animationDuration);
...
describe("Alert Dialog", () => {
    ...
    it("Upon animation end 'Continue' button's background color should change", async () => {
        const continueBtnColor = await page.$eval("#continue", el => getComputedStyle(el).backgroundColor);
        await page.hover("#continue");
        const continueBtnColorAfterHover = await page.$eval("#continue", el => getComputedStyle(el).backgroundColor);
